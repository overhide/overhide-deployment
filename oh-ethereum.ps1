...
switch (choose "(t)est (p)rod (s)tage?" "t","p","s") {
  "t" {
    $PROPS_FILE = ".\config\oh-ethereum-test.props";
    $SECRETS_FILE = ".\secrets\oh-ethereum-test.secrets.yaml";
    $STATUS_URL = "https://rinkeby.ethereum.overhide.io/status.json";
  }
  "p" {
    $PROPS_FILE = ".\config\oh-ethereum-prod.props";
    $SECRETS_FILE = ".\secrets\oh-ethereum-prod.secrets.yaml";
    $STATUS_URL = "https://ethereum.overhide.io/status.json";
  }
  "s" {
    $PROPS_FILE = ".\config\oh-ethereum-stage.props";
    $SECRETS_FILE = ".\secrets\oh-ethereum-stage.secrets.yaml";
    $STATUS_URL = "https://rinkeby-stage.ethereum.overhide.io/status.json";
  }
}
...
switch (choose "`n`n do (i)mages `n do (s)ettings `n (d)eployment `n (e)xec in `n (r)edeploy `n (f)ollow logs for deployment `n s(t)atus `n`n Choose one..." "i", "s", "d", "e", "r", "f", "t") {
  ...
  "d" {
    go `
    "Apply oh-ethereum?" `
    (genKubectlApply "k8s/oh-ethereum.yaml" $conf);
